...
async function main() {
  ...
  const confirmed = [];
  const needValidate = [];
  const removed = { year: 0, titleBlacklist: 0, mainland: 0, noGenre: 0 };
  ...
  console.log(`  移除: 年份=${removed.year} 黑名单=${removed.titleBlacklist} 大陆=${removed.mainland} 无标签=${removed.noGenre}`);
